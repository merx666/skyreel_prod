...
@section('title', __('Edytuj sprzęt'))
    ...
    <div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-8">
            <div class="flex items-center mb-4">
                ...
                <h1 class="text-3xl font-bold text-white">{{ __('Edytuj sprzęt') }}</h1>
            </div>
            <p class="text-gray-400">{{ __('Zaktualizuj informacje o sprzęcie') }}</p>
        </div>

        <!-- Form -->
        <div class="liquid-glass p-8 rounded-lg">
            <form action="{{ route('equipment.update', $equipment) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <select name="type" id="type" required
                        ...
                        <option value="drone" {{ (old('type', $equipment->type) == 'drone') ? 'selected' : '' }}>{{ __('Dron') }}</option>
                        <option value="camera" {{ (old('type', $equipment->type) == 'camera') ? 'selected' : '' }}>{{ __('Kamera') }}</option>
                        <option value="lens" {{ (old('type', $equipment->type) == 'lens') ? 'selected' : '' }}>{{ __('Obiektyw') }}</option>
                        <option value="accessory" {{ (old('type', $equipment->type) == 'accessory') ? 'selected' : '' }}>{{ __('Akcesoria') }}</option>
                    </select>
                    @error('type')
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="4"
                              class="w-full px-4 py-3 bg-gray-800 border border-gray-700 rounded-lg text-white focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                              placeholder="{{ __('Dodatkowe informacje o sprzęcie...') }}">{{ old('description', $equipment->description) }}</textarea>
                    @error('description')
                        <p class="text-red-400 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    <label for="image" class="block text-sm font-medium text-gray-300 mb-2">
                        {{ $equipment->image_url ? __('Nowe zdjęcie (opcjonalne)') : __('Zdjęcie sprzętu') }}
                    </label>
