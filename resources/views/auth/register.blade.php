...
<div class="min-h-screen flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
    <div class="max-w-md w-full space-y-8">
        <div class="text-center">
            <h2 class="text-3xl font-bold">Utwórz konto</h2>
            <p class="mt-2 text-secondary">Lub <a href="{{ route('login') }}" class="text-accent hover:underline">zaloguj się</a></p>
        </div>
        ...
        <form method="POST" action="{{ route('register') }}" class="space-y-6">
            ...
            <div>
                <label for="role" class="block text-sm font-medium mb-2">Typ konta</label>
                <select id="role" name="role" required class="input-field w-full">
                    <option value="">Wybierz typ konta</option>
                    <option value="client" {{ old('role') == 'client' ? 'selected' : '' }}>
                        Klient - szukam operatora drona
                    </option>
                    <option value="operator" {{ old('role') == 'operator' ? 'selected' : '' }}>
                        Operator - oferuję usługi dronowe
                    </option>
                </select>
            </div>
            ...
            <div>
                ...
                <input id="password" name="password" type="password" required 
                       class="input-field w-full" 
                       placeholder="••••••••">
                <p class="mt-1 text-xs text-secondary">Minimum 8 znaków</p>
            </div>

            <div>
                <label for="password_confirmation" class="block text-sm font-medium mb-2">Potwierdź hasło</label>
            ...
            <div class="flex items-center">
                ...
                <label for="terms" class="ml-2 block text-sm text-secondary">
                    Akceptuję <a href="#" class="text-accent hover:underline">regulamin</a> i 
                    <a href="#" class="text-accent hover:underline">politykę prywatności</a>
                </label>
            </div>
